...
% Technical Contribution: What's the key technical idea to solve the problem? Why is it beautiful?
% Applications / Future Work: What will your solution enable? How does it project into the future? How will it inspire future work?
We identify a number of contributions in our research. 
(1) We demonstrate that convolutional models can be effectively employed for tracking of geometry in motion. Note how although we focus on modelling hands, our research paves a new way of to track any form of articulated body. 
(2) As many possible variants of convolution skeletons exist, it is essential to identify a skeletal topology that performs optimally for hands; see \Figure{topology} \AT{won't they us to compare against other topologies?}. 
...
(4) We introduce an optimization technique for convolution models that adapts a given template to a specific user, and demonstrate how this results in substantial improvements in tracking precision.
(5) Rather than relying on a artist-built rig we automatically 
