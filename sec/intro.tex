...
\todo{While the integral in \Eq{convsurf} might be difficult to evaluate, distances to $\surface$ can conveniently be computed by querying distances to the piecewise linear structures composing $\skeleton$; see \Figure{convsurf}.}
...
\AT{\Section{overview}?}
Section~\ref{sec:model} then introduces our convolution surface model that allows modeling human hands at high geometric fidelity for improved tracking accuracy. We show how our novel formulation enables efficient correspondence computation crucial for realtime tracking \AT{not true?}.
