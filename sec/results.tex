...
\section{Results}
\label{sec:results}
We evaluate our technique on a variety of sequences, across a number of users \Anastasia{across just one user} and performed qualitative as well as quantitative  \Anastasia{only qualitative for all except for Htrack} evaluation of our method to the state-of-the-art~\cite{sridhar2015fast,tagliasacchi2015robust,sharp2015accurate,taylor2016concerto} \Anastasia{Also Qian et al}. We also propose new algorithm-agnostic metrics tailored to high-precision tracking evaluation, and release the new \handy{} dataset associated with these metrics.
...
We create the new \handy{} tracking dataset for the evaluation of high-precision generative tracking algorithms. Our dataset contains \todo{$\approx$10m} of recorded sequences from an \realsense{} SR300 sensor. The dataset is designed to cover the entire range of motions that has been surveyed in recent techniques. As detailed in \Figure{motiontypes}, we identified three main axes of variation \Anastasia{I had in mind ``axes of complexity``. The larger coordinate at each axis the more difficult is the motion to track with monocular depth sensor.} in the hand tracking literature, and devised the \textsc{teaser} dataset to thoroughly sample this space; see~\Video{00:00}. 
