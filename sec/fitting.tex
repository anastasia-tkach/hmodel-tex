...
\label{eq:rigidity}
\end{equation} 
Note that only a subset of the edges of our control skeleton, as illustrated in \Figure{topology}, are required to satisfy this rigidity requirement. At the cost of introducing auxiliary variables $R_{i}$, this energy becomes independent from pose $\parposes$ and posture $\parposture$ angles. In \Section{optimization} we detail how this leads to an efficient optimization scheme for \Equation{step2}. 
...
% 
\begin{equation}
E_{\text{valid}} = \sum_{\ballcenter_k \in \skeleton} \chi(\ball_k) \proj_{\model \setminus \ball_k}(\ballcenter_k) 
\end{equation}
% 
...
\subsection{Optimization \TODO{Andrea}}
\label{sec:optimization}

\paragraph{Step 1: pose and posture}
similar to real-time tracking
\paragraph{Step 2: geometry}
ARAP-like solve

\TODO{Mention gradients in the appendix? especially for the ones in \Eq{step2}}.
