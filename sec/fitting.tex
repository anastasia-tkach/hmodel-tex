...
The rest-pose geometry of a convolution model is fully specified by two matrices specifying the set of sphere positions $\centers$ and radii $\radii$. The geometry is then posed through the application of  kinematic chain transformations. Given a point $\bar\point$ on the model $\model$ at rest pose, its 3D position after the model has been posed can be computed by evaluating the expression:
% 
\begin{equation}
