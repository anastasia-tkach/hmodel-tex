...
% Discriminative
Conversely, discriminative methods estimate the pose by extracting features from each image independently by learning from a large dataset of annotated exemplars~\cite{keskin2012hand,tang2013real,tejani2014latent,sun2015cascaded}. While discriminative methods are avoid drift, they lack the accuracy of generative methods. \AT{also the joint estimates might violate kinematics?}
% Hybrid
...
\MP{We should add a sentence to summarize why our model is different and better. Can we say some thing like: The convolution model that we propose offers high geometry accuracy comparable to dense triangle meshes, while retaining a compact representation for efficient correspondence queries and effective user adaptation.}
...
In contrast to explicit polygonal modeling, sculpting implicit surfaces provides a more natural way of designing complex geometry \MP{that's probably a strong statement that many people would disagree with. Most modelers are still explicit.  Maybe rephrase to:
Implicit sculpting tools have recently become a viable alternative to mesh- or spline-based approaches for modeling complex geometries.}
...
\MP{This is a bit counterintuitive. You say it's intuitive for a human and that makes it easy to automate. Most tasks that are intuitive for a human (e.g. catch a ball) are hard to automate. I guess what you want to say is that the parameters are "natural" which makes it easy for both humans and algorithms to optimize? Not sure how to best phrase this.}
...
Tracking with an implicit model has also received little attention in the literature, with \cite{fua2003soft} being a notable exception. \MP{the previous sentence seems a bit out of place in this paragraph. Should we move it to one of the previous ones?}
