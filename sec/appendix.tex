...
\paragraph{Computation of $\skewproj$ from \Figure{corresp}-(left)}
\label{app:pillcorr}
% 
\AT{Can we move most of the details here, by using the ray-intersection strategy? See below}

\paragraph{Computation of $\skewproj$ from \Figure{corresp}-(right)}
\label{app:wedgecorr}
\AT{Could we re-write this by using the ``ray intersection'' I suggested? Then the only thing I need from the appendix is the computation of a plane tangent to three spheres.}
% 
\begin{DRAFT}
